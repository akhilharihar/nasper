...
void tud_hid_set_report_cb(uint8_t itf, uint8_t report_id, hid_report_type_t report_type, uint8_t const* buffer, uint16_t bufsize) {
  ...
  if(vec.size() == 8) {
  global_ui->update_info(vec[0].c_str(), vec[1].c_str(), vec[2].c_str(), vec[3].c_str(), vec[4].c_str(), std::stoi(vec[5].c_str()), std::stoi(vec[6].c_str()), std::stoi(vec[7].c_str()));
  }

  // uart_puts(uart0, fdsf.c_str());
